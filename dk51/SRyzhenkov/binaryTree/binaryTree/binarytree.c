...
void deleteTree(TreePtr aTree) {
	if (NULL != aTree) {
		if (NULL != aTree->root->leftChild) delete(aTree->root->leftChild);
		if (NULL != aTree->root->rightChild) delete(aTree->root->rightChild);
	}
	free(aTree);
}

void delete(NodePtr inputNode) {
	if (NULL != inputNode->leftChild) delete(inputNode->leftChild);
	if (NULL != inputNode->rightChild) delete(inputNode->rightChild);
	free(inputNode->note);
	free(inputNode);
}
