...
unsigned oa_delete(ordArrayPtr anArray, int aValue, int size)
{
	unsigned theResult = oa_find(anArray, size, aValue);
...
unsigned oa_find(int Array[], int size, int aValue)
{
	unsigned theResult = size;
	unsigned theLowerBound = 0;
	unsigned theUpperBound = size - 1;
	unsigned theCurIn = 0;
	
	while (1)
	{
		theCurIn = (theLowerBound + theUpperBound) / 2;
		if (Array[theCurIn] == aValue) //<!- an element was found
		{
			theResult = theCurIn;
			break;
		}
