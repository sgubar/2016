...
IntNode *SLInsertNodeAtIndex(IntList *aList, IntNode *aNewNode, int anIndex)
{
	...
	else
	{
		...
		{
			IntNode *PreNode = SLNodeAtIndex(aList, anIndex - 1);
			...
			{
				aNewNode->nextNode = PreNode->nextNode;
			}
			PreNode->nextNode = aNewNode;
			aList->count += 1;
			return(aNewNode);
		}
	}
...
IntNode *SLRemovedNodeAtIndex(IntList *aList, int anIndex)
{
	...
	else
	{
		...
		else
		{
			...
			PreNode->nextNode = RemNode->nextNode;
			aList->count -= 1;
			return(RemNode);
		}
