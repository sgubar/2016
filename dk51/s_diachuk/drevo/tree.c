...
void deleteTree(TreePtr aTree)//видалення
{
...
int CountTree(NodePtr k)
{
	int result = 0;
if (k != NULL)
{
	result++;
	result += CountTree(k->leftChild);
	result += CountTree(k->rightChild);}
return result;
}

void CleanTree (NodePtr k )
//Очистка дерева.
//*k - указатель на корень дерева.
{
if  (k!=NULL)
{ CleanTree (k->leftChild);
    CleanTree (k->rightChild);
   free(k); }
}

void Vyvod (NodePtr k ,int l)
...
int i;
 if  (k!=NULL)
{ Vyvod (k->rightChild,l+1);
 for  (i=1; i<=l; i++) printf(" ");
printf("%s \n",k->note->number);
Vyvod (k->leftChild,l+1); }
}
