...
void deleteLeaf(TreePtr aTree, const char *aName)
{
	...
	while (0 != (theCompare = strcmp(aName, theCurrent->note->name)))
	{
		...
		if (theCompare < 0)
		{
			theLeftChild = 1; /*that`s a left node*/
			theCurrent = theCurrent->leftChild;
		}
		else
		{
			theLeftChild = 0; /*that`s a right node*/
			theCurrent = theCurrent->rightChild;
		}

		if (NULL == theCurrent) /*this node doesn`t exist*/
		{
			return;
		}
	}

	if (NULL == theCurrent->leftChild && NULL == theCurrent->rightChild) /*if this node don`t have any child*/
	{
		if (aTree->root == theCurrent)
		{
			aTree->root = NULL;
		}
		else if (theLeftChild)
		{
...
void deleteTree(TreePtr aTree, NodePtr aNode)
{
	...
	deleteLeaf(aTree, aNode->note->name);/*firstly delete left node (the lowest)*/
	}
	else
	return;
}
...
NodePtr findNode(TreePtr aTree, const char *aName)
{
	...
	while (0 != (theComparisionResult = strcmp(aName, theCurrentNode->note->name))) //<! - walk through the tree
	{
		theCurrentNode = (theComparisionResult < 0)
					? theCurrentNode->leftChild // currentNode=leftChild if the currentNode !=0;
...
void insertNode(TreePtr aTree, char *aName, char *aPhoneNumber)
{
	...
	memset(theNewNode, 0, sizeof(Node)); /*fill theNewNode with 0*/
