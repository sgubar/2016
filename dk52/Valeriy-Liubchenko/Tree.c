...
result = displaySide(aTree->root->leftChild, 0);

if(NULL != aTree->root->rightChild)
result = displaySide(aTree->root->rightChild, 0);

}
return result;
}
int displaySide(NodePtr currentNode)
{
...
displaySide(currentNode->leftChild);

}
if(NULL != currentNode->rightChild)
{
// printf("rightchild: %s = %s\n",currentNode->note->name,currentNode->note->number);
displaySide(currentNode->rightChild);
