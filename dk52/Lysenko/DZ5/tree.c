...
int countTree(TreePtr aTree) {	//vischitivaem kol-vo nod v dereve
	int count = 0;
	Node *root = aTree->root;

	int TreePass(Node* root) {
		int count = 0;

		if (root) {
			count++;
			printf("\n%s: %s ", root->note->name, root->note->number);

			count = count + TreePass(root->leftChild);

			count = count + TreePass(root->rightChild);
...
void deleteTree(TreePtr aTree) // udalenie elementov v dereve 
{
	Node **root = aTree;
	if (*root) {
