    ...
    void DeleteSimilar(List* theList)
{
    ...
    void SelectionSort(List* theList)
{
    if(theList!=NULL)
    {
        Node* theNode;
        Node*theMinNode;
        Node*shifterNode;
        theMinNode=theNode=shifterNode=theList->head;
        while(theNode!=NULL)
        {
            shifterNode=theNode;//start with the left bound
            while(shifterNode!=NULL)
            {
                if(shifterNode->value < theMinNode->value)//find the minimal node
                    theMinNode=shifterNode;
                shifterNode=shifterNode->nextptr;//go to the next node
            }        

            int tempVal=theNode->value;//swap elements
            theNode->value=theMinNode->value;
            theMinNode->value=tempVal;

            theNode=theNode->nextptr;//shifting left bound
        }
