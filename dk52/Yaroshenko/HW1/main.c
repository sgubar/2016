...
int main() {
	...
	head=PrintRandomList();                                               //create list of random 10 number
	printf("\nWhat is index of a new node?\n");
	scanf("%d",&n);
	if(n<1 || n>11){                                                      //checking index
	...
	scanf("%d",&value);
	newhead=AddNodeToList(head, n, value);                                //insertion of the node
	
	printf("What is the index of node you would like to remove?\n");      
	scanf("%d",&m);
	if(m<1 || m>12){                                                      //checking index
	  printf("You pick the wrong index!\n");
	  system("pause");
	  exit(0);
	}
	removed=RemoveNode(newhead,m);                                        //removing node
